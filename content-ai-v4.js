...
async function runAI(){
 ...
 try{
   ...
   activePackage=r.data;
   const reelIds=[...(r.data.main_reel?.scenes||[]).map(s=>s.media_asset_id||s.media_id),...(r.data.teaser_reel?.scenes||[]).map(s=>s.media_asset_id||s.media_id)].filter(Boolean);
   await ensurePreviewMedia(reelIds);
   renderAIResult(r.data);
   renderDraftPackage(r.data.drafts||[],r.data);
   renderReleaseWorkspace(r.data);
   await loadProjectState();
   notify('AI release package created and queued for human review.');
 }catch(e){
...
function openReleaseWorkspace(){
 modal.classList.remove('open');
 const ws=document.getElementById('aiReleaseWorkspace');
...
function renderAIResult(data){
 ...
 const main=data.main_reel,teaser=data.teaser_reel;
 document.getElementById('aiContentBody').innerHTML='<div class="ai-result"><div class="ai-summary"><h4>Release package ready</h4><p><strong>'+esc(data.creative_hook||'Dream Hunt story')+'</strong><br>'+esc(data.story_summary||'')+'</p>'+(main&&teaser?'<div style="display:flex;gap:7px;flex-wrap:wrap;margin-top:10px"><span class="badge lime">MAIN REEL · '+Math.round(Number(main.duration_seconds||30))+'s</span><span class="badge">TEASER · '+Math.round(Number(teaser.duration_seconds||9))+'s</span><span class="badge">'+(data.drafts?.length||0)+' SOCIAL ITEMS</span></div>':'')+'</div><div class="ai-summary"><h4>Recommended assets</h4><div class="ai-assets">'+(recs.length?recs.map(r=>'<div class="ai-pick"><span class="badge">'+esc(r.role||'supporting')+'</span><div class="ai-score">'+Math.round(Number(r.score||0))+'</div><b>'+esc(mediaName(r.media_id))+'</b><span>'+esc(r.reason||'')+'</span></div>').join(''):'<p>No visual ranking returned.</p>')+'</div></div>'+(notes.length?'<div class="ai-summary"><h4>Permission / review notes</h4>'+notes.map(n=>'<div class="ai-question">'+esc(n)+'</div>').join('')+'</div>':'')+(questions.length?'<div class="ai-summary"><h4>Facts to confirm before publishing</h4>'+questions.map(q=>'<div class="ai-question">'+esc(q)+'</div>').join('')+'</div>':'')+'<div class="ai-summary"><h4>Internal recap</h4><p>'+esc(data.recap||'')+'</p></div><div style="display:flex;justify-content:flex-end;gap:8px;flex-wrap:wrap"><button class="btn" id="aiCloseResult">Close</button><button class="btn accent" id="aiPreviewPackage">▶ Preview reels</button><button class="btn primary" id="aiOpenQueue">Review drafts</button></div></div>';
 document.getElementById('aiCloseResult').onclick=()=>modal.classList.remove('open');
 document.getElementById('aiPreviewPackage').onclick=openReleaseWorkspace;
...
function renderDraftPackage(drafts,data){
 ...
 const order=['teaser','release','follow_up','donor','sponsor','evergreen'];
 const labels={teaser:'TEASER',release:'MAIN RELEASE',follow_up:'FOLLOW-UP',donor:'DONOR / INTERNAL',sponsor:'SPONSOR',evergreen:'EVERGREEN'};
 box.innerHTML=order.map(stage=>{
   const rows=drafts.filter(d=>(d.release_stage||'release')===stage);if(!rows.length)return '';
   return '<div style="margin-bottom:12px"><div style="font-size:9px;font-weight:900;letter-spacing:.08em;color:var(--muted);margin-bottom:6px">'+labels[stage]+'</div>'+rows.map(d=>'<div class="post"><div class="posthead"><span class="platform">'+esc(d.platform||'Social')+'</span><span class="badge">'+esc(String(d.content_type||'draft').replaceAll('_',' ').toUpperCase())+'</span></div><p>'+esc(d.body||'')+'</p>'+(d.call_to_action?'<p style="font-size:10px;color:var(--muted)"><b>CTA:</b> '+esc(d.call_to_action)+'</p>':'')+'</div>').join('')+'</div>';
 }).join('');
 const status=document.getElementById('draftStatus');if(status)status.textContent=drafts.length+' AI deliverable'+(drafts.length===1?'':'s')+' — teaser, release and follow-up package';
...
if(gen){gen.textContent='✦ Generate AI Package';gen.onclick=openBuilder}
const top=document.getElementById('aiGenerateTop');if(top){top.textContent='✦ Generate AI Package';top.onclick=openBuilder}
const wsGen=document.getElementById('aiWorkspaceGenerate');if(wsGen)wsGen.onclick=openBuilder;
const wsRefresh=document.getElementById('aiWorkspaceRefresh');if(wsRefresh)wsRefresh.onclick=loadProjectState;
...
window.DreamHuntContentAI={open:openBuilder,refresh:loadActivities,preview:openReleaseWorkspace};
})();