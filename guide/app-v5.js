...
let user=null,guide=null,weekends=[],weekendId=null,hunts=[],assignments=[],currentHunt=null,messages=[],channel=null,watchId=null,lastPingAt=0,pendingPhone=null,lastCodeSentAt=0,audioCtx=null,notifyEnabled=localStorage.getItem('dh_guide_notifications')==='1';
...
async function enableGuideNotifications(){
 try{
  if('Notification' in window&&Notification.permission!=='granted'){
   const p=await Notification.requestPermission();
   if(p!=='granted'){toast('Notifications were not enabled.');return}
  }
  notifyEnabled=true;localStorage.setItem('dh_guide_notifications','1');
  unlockGuideAudio(true);
  updateNotifyButton();
  toast('Message notifications and sound enabled.');
 }catch(e){toast('Could not enable notifications: '+(e.message||e))}
}
function unlockGuideAudio(test=false){
 try{
  const AC=window.AudioContext||window.webkitAudioContext;if(!AC)return;
  if(!audioCtx)audioCtx=new AC();if(audioCtx.state==='suspended')audioCtx.resume();
  if(test)playGuideChime();
 }catch{}
}
function playGuideChime(){
 try{
  unlockGuideAudio(false);if(!audioCtx||audioCtx.state!=='running')return;
  const now=audioCtx.currentTime;
  [[880,0,.10],[1175,.13,.13]].forEach(([freq,delay,dur])=>{
   const o=audioCtx.createOscillator(),g=audioCtx.createGain();
   o.type='sine';o.frequency.value=freq;g.gain.setValueAtTime(.0001,now+delay);g.gain.exponentialRampToValueAtTime(.16,now+delay+.01);g.gain.exponentialRampToValueAtTime(.0001,now+delay+dur);
   o.connect(g).connect(audioCtx.destination);o.start(now+delay);o.stop(now+delay+dur+.02);
  });
 }catch{}
}
async function showGuideSystemNotification(title,body){
 if(!notifyEnabled||!('Notification' in window)||Notification.permission!=='granted')return;
 try{
  if('serviceWorker' in navigator){
   const reg=await navigator.serviceWorker.ready;
   await reg.showNotification(title,{body,icon:'icon.svg',badge:'icon.svg',tag:'dh-guide-chat',renotify:true,vibrate:[120,60,120]});
...
function updateNotifyButton(){
 ...
 b.textContent=notifyEnabled&&granted?'Notifications On':'Enable Notifications';
 b.className='btn '+(notifyEnabled&&granted?'accent':'');
}
function handleGuideIncomingMessage(msg){
 if(!msg||!user||msg.sender_user_id===user.id)return;
 if(!notifyEnabled)return;
 playGuideChime();
 if(navigator.vibrate)navigator.vibrate([100,60,100]);
 const title=msg.hunt_id?(hunts.find(h=>h.id===msg.hunt_id)?.title||'Dream Hunt Message'):'Dream Hunt Message';
 const body=String(msg.message||'New message').slice(0,180);
 showGuideSystemNotification(title,body);
}
document.addEventListener('pointerdown',()=>{if(notifyEnabled)unlockGuideAudio(false)},{once:true});
function show(id){
...
function subscribeRealtime(){
 ...
 channel=sb.channel('guide-mobile-'+weekendId)
  .on('postgres_changes',{event:'INSERT',schema:'public',table:'dh_hunt_messages',filter:'weekend_id=eq.'+weekendId},payload=>{handleGuideIncomingMessage(payload.new);loadChat()})
  .on('postgres_changes',{event:'*',schema:'public',table:'dh_hunt_assignments'},()=>loadHunts())
  .subscribe();
}
...
$('enableNotificationsBtn').onclick=enableGuideNotifications;updateNotifyButton();
