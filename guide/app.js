...
let user=null,guide=null,weekends=[],weekendId=null,hunts=[],assignments=[],currentHunt=null,messages=[],channel=null,watchId=null,lastPingAt=0,authMode='signin';
...
function authUi(mode){
 authMode=mode;$('signInTab').className='btn'+(mode==='signin'?' primary':'');$('createTab').className='btn'+(mode==='signup'?' primary':'');
 $('authBtn').textContent=mode==='signin'?'Sign In':'Create Account';
}
$('signInTab').onclick=()=>authUi('signin');$('createTab').onclick=()=>authUi('signup');
...
$('authBtn').onclick=async()=>{
 ...
 try{
  let r;
  if(authMode==='signin')r=await sb.auth.signInWithPassword({email,password});
  else r=await sb.auth.signUp({email,password});
  if(r.error)throw r.error;
  if(authMode==='signup'&&!r.data.session){toast('Account created. Check your email if confirmation is required, then sign in.');authUi('signin');return}
  await enterApp(r.data.user||r.data.session?.user);
 }catch(e){toast(e.message||'Authentication failed.')}
 finally{$('authBtn').disabled=false;$('authBtn').textContent=authMode==='signin'?'Sign In':'Create Account'}
};
...
async function enterApp(u){
 ...
 try{
  ...
  toast(e.message||'No active guide invitation was found for this email.');
 }
}
...
async function uploadFiles(files){
 ...
 const preview=$('mediaPreview');preview.innerHTML=files.slice(0,9).map(f=>'<div class="preview">'+(f.type.startsWith('image/')?'<img src="'+URL.createObjectURL(f)+'">':'▶')+'</div>').join('');
 for(let i=0;i<files.length;i++){
  const f=files[i];if(f.size>75*1024*1024){toast(f.name+' is over the 75 MB mobile limit.');continue}
  $('uploadProgress').textContent='Uploading '+(i+1)+' of '+files.length+'…';
  const safe=f.name.replace(/[^a-zA-Z0-9._-]+/g,'-').slice(-100);
  const path=guide.org_id+'/guide-intake/'+currentHunt.id+'/'+user.id+'/'+Date.now()+'-'+crypto.randomUUID()+'-'+safe;
  const up=await sb.storage.from('dh-trial-media').upload(path,f,{contentType:f.type,cacheControl:'3600',upsert:false});
  if(up.error){toast('Upload failed: '+up.error.message);continue}
  const row=await sb.from('dh_media_assets').insert({org_id:guide.org_id,activity_id:currentHunt.activity_id,permission_id:null,storage_path:path,media_type:f.type,caption:f.name,sensitive_content:false,approved_for_social:false,approved_for_sponsor_use:false,metadata:{original_name:f.name,size:f.size,mime:f.type,source:'guide-mobile',hunt_id:currentHunt.id},created_by:user.id});
  if(row.error){await sb.storage.from('dh-trial-media').remove([path]);toast('Media record failed: '+row.error.message)}
 }
 $('uploadProgress').textContent='Upload complete. Dream Hunt will review all media before use.';$('mediaInput').value='';toast('Media sent to Dream Hunt.');
}
...
$('signOutBtn').onclick=async()=>{stopTracking();if(channel)sb.removeChannel(channel);await sb.auth.signOut();user=guide=null;weekends=[];hunts=[];assignments=[];currentHunt=null;$('bottomNav').classList.add('hidden');$('topSub').textContent='Volunteer field app';show('authScreen')};
