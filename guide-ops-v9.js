...
let realtimeChannel=null, reloadTimer=null, editingWeekendId=null, editingHuntId=null, editingGuideId=null, assignmentHuntId=null,goAudioCtx=null,goNotifyEnabled=localStorage.getItem('dh_ops_notifications')==='1';
...
function normalizeUSPhone(raw){const d=String(raw||'').replace(/\D/g,'');if(d.length===10)return '+1'+d;if(d.length===11&&d.startsWith('1'))return '+'+d;if(String(raw||'').trim().startsWith('+')&&d.length>=8&&d.length<=15)return '+'+d;return null}
...
section.innerHTML=
'<div class="section-title"><div><h3>Guide Ops</h3><p>Load every hunt, authorize guides, receive media, monitor GPS/emergencies and communicate during the weekend.</p></div><div class="go-actions"><span class="badge" id="goSmsStatus">SMS: checking…</span><button class="btn" id="goCopyInvite">Copy Guide Invite</button><button class="btn" id="goOpenMobile">Open Guide App</button><button class="btn" id="goBulkGuides">Bulk Import Guides</button><button class="btn" id="goAddGuide">+ Guide</button><button class="btn" id="goAddHunt">+ Hunt</button><button class="btn primary" id="goAddWeekend">+ Hunt weekend</button></div></div>'+
...
const guideModal=makeModal('goGuideModal','Volunteer guide','<div class="field"><label>DISPLAY NAME</label><input id="goGName"></div><div class="formrow"><div class="field"><label>MOBILE NUMBER</label><input id="goGPhone" type="tel" inputmode="tel" placeholder="(318) 555-1234"></div><div class="field"><label>EMAIL (OPTIONAL)</label><input id="goGEmail" type="email"></div></div><div class="field"><label>NOTES</label><textarea id="goGNotes"></textarea></div><label style="font-size:11px"><input id="goGActive" type="checkbox" checked> Active guide</label><div class="go-actions" style="justify-content:flex-end;margin-top:12px"><button class="btn primary" id="goGSave">Save guide</button></div><p class="go-modal-note">The mobile app will securely claim this guide profile when the volunteer signs in using the same email address.</p>');
const bulkModal=makeModal('goBulkModal','Bulk Import Volunteer Guides','<p class="go-modal-note" style="margin-top:0">Paste one guide per line. Use <b>Name, Mobile Number</b> or <b>Name, Mobile Number, Email</b>. Tabs also work if you paste from Excel or Sheets.</p><div class="field"><label>GUIDE ROSTER</label><textarea id="goBulkText" style="min-height:260px" placeholder="John Smith, 318-555-1234\nMary Jones, 318-555-5678, mary@example.com"></textarea></div><label style="font-size:11px"><input id="goBulkAuthorize" type="checkbox" checked> Authorize imported guides for the currently selected hunt weekend</label><div class="go-actions" style="justify-content:flex-end;margin-top:12px"><button class="btn primary" id="goBulkSave">Import Guides</button></div><div id="goBulkResult" class="go-modal-note"></div>');
...
document.getElementById('goCopyInvite').onclick=async()=>{
  const msg='Dream Hunt volunteer guides: open the Guide App here: https://jaysonrayrichardson-bot.github.io/dreamhunt-growth-agent/guide/ — enter the mobile number Dream Hunt has on file and use the 6-digit code texted to you. No password needed.';
  try{await navigator.clipboard.writeText(msg);notice('Guide invite copied. You can text the same message to the entire guide group.')}catch{prompt('Copy this guide invite:',msg)}
};
checkSmsStatus();
document.getElementById('goBulkGuides').onclick=()=>{document.getElementById('goBulkText').value='';document.getElementById('goBulkResult').textContent='';bulkModal.classList.add('open')};
document.getElementById('goBulkSave').onclick=bulkImportGuides;
...
document.getElementById('goEnableNotifications').onclick=enableOpsNotifications;updateOpsNotifyButton();
...
async function checkSmsStatus(){
  const el=document.getElementById('goSmsStatus');if(!el)return;
  try{
    const r=await fetch('https://dlcahaelopldckwguzru.supabase.co/auth/v1/settings',{headers:{apikey:'sb_publishable_hgtbaNhPcZAOZc4ZKMmoQw_DhuVJG6i'}});
    const s=await r.json();
    const enabled=!!s?.external?.phone;
    el.textContent=enabled?'SMS LOGIN READY':'SMS LOGIN NEEDS SETUP';
    el.className='badge '+(enabled?'lime':'orange');
    el.title=enabled?'Phone OTP is enabled.':'Enable Phone Auth and configure the SMS provider in Supabase Auth.';
  }catch(e){el.textContent='SMS STATUS UNKNOWN';el.className='badge'}
}
...
async function enableOpsNotifications(){
 try{
  if('Notification' in window&&Notification.permission!=='granted'){
   const p=await Notification.requestPermission();
   if(p!=='granted'){notice('Notifications were not enabled.');return}
  }
  goNotifyEnabled=true;localStorage.setItem('dh_ops_notifications','1');
  unlockOpsAudio(true);updateOpsNotifyButton();notice('Guide chat notifications and sound enabled.');
 }catch(e){notice('Could not enable notifications: '+(e.message||e))}
}
function unlockOpsAudio(test=false){
 try{
  const AC=window.AudioContext||window.webkitAudioContext;if(!AC)return;
  if(!goAudioCtx)goAudioCtx=new AC();if(goAudioCtx.state==='suspended')goAudioCtx.resume();
  if(test)playOpsChime();
 }catch{}
}
function playOpsChime(){
 try{
  unlockOpsAudio(false);if(!goAudioCtx||goAudioCtx.state!=='running')return;
  const now=goAudioCtx.currentTime;
  [[740,0,.10],[988,.13,.14]].forEach(([freq,delay,dur])=>{
   const o=goAudioCtx.createOscillator(),g=goAudioCtx.createGain();
   o.type='sine';o.frequency.value=freq;g.gain.setValueAtTime(.0001,now+delay);g.gain.exponentialRampToValueAtTime(.14,now+delay+.01);g.gain.exponentialRampToValueAtTime(.0001,now+delay+dur);
   o.connect(g).connect(goAudioCtx.destination);o.start(now+delay);o.stop(now+delay+dur+.02);
  });
 }catch{}
}
function updateOpsNotifyButton(){
 const b=document.getElementById('goEnableNotifications');if(!b)return;
 const granted=('Notification' in window&&Notification.permission==='granted');
 b.textContent=goNotifyEnabled&&granted?'Notifications On':'Enable Notifications';
 b.className='btn small '+(goNotifyEnabled&&granted?'accent':'');
}
async function showOpsSystemNotification(title,body){
 if(!goNotifyEnabled||!('Notification' in window)||Notification.permission!=='granted')return;
 try{new Notification(title,{body,icon:'guide/icon.svg',tag:'dh-ops-chat'});}catch{}
}
function handleOpsIncomingMessage(msg){
 if(!msg||!liveUser||msg.sender_user_id===liveUser.id)return;
 if(!goNotifyEnabled)return;
 playOpsChime();if(navigator.vibrate)navigator.vibrate([100,50,100]);
 const sender=msg.guide_id?guideName(msg.guide_id):'Dream Hunt';
 const title=msg.message_type==='emergency'?'Dream Hunt Emergency Message':'New message from '+sender;
 const scope=msg.hunt_id?huntName(msg.hunt_id):'All guides';
 showOpsSystemNotification(title,scope+' — '+String(msg.message||'').slice(0,160));
}
document.addEventListener('pointerdown',()=>{if(goNotifyEnabled)unlockOpsAudio(false)},{once:true});
...
function renderRoster(){
  ...
  box.innerHTML=guides.map(g=>{
    ...
    return '<div class="go-guide" style="'+(!g.active?'opacity:.55':'')+'"><div class="grow"><b>'+E(g.display_name)+'</b><small>'+E(g.phone||'No mobile')+(g.email?' · '+E(g.email):'')+' · '+(g.auth_user_id?'Mobile linked':'Not linked yet')+(mine.length?' · '+mine.length+' hunt assignment'+(mine.length===1?'':'s'):'')+'</small></div>'+
...
function renderMedia(){
  ...
  box.innerHTML=media.map(x=>{
    ...
    return '<div class="go-media"><div class="go-media-thumb" data-preview-media="'+x.id+'">'+preview+'</div><div class="go-media-info"><b>'+E(x.caption||x.metadata?.original_name||'Guide media')+'</b><small>'+E(h?.title||'Hunt')+' · '+E(g?.display_name||'Guide')+'<br>'+fmt(x.created_at)+'</small><div class="go-actions" style="margin-top:6px"><button class="btn small" data-media-content="'+x.id+'">Content</button><button class="btn small go-danger" data-media-delete="'+x.id+'">Delete</button></div></div></div>';
  }).join('');
  ...
  box.querySelectorAll('[data-media-delete]').forEach(b=>b.onclick=()=>deleteMedia(b.dataset.mediaDelete));
}
async function deleteMedia(id){
  const x=byId(media,id);if(!x)return;
  const label=x.caption||x.metadata?.original_name||'this media';
  if(!confirm('Permanently delete '+label+'? This also removes it from any content draft using it.'))return;
  try{
    if(x.storage_path){
      const sr=await sb.storage.from('dh-trial-media').remove([x.storage_path]);
      if(sr.error)throw new Error('Storage delete failed: '+sr.error.message);
    }
    const dr=await sb.from('dh_media_assets').delete().eq('org_id',liveOrgId).eq('id',id);
    if(dr.error)throw new Error('Media record delete failed: '+dr.error.message);
    media=media.filter(m=>m.id!==id);
    renderMedia();renderStats();
    if(mediaModal.classList.contains('open'))mediaModal.classList.remove('open');
    notice('Media deleted permanently.');
  }catch(e){
    notice('Could not delete media: '+(e.message||e));
  }
}
function openMedia(id){
  const x=byId(media,id);if(!x)return;const video=(x.media_type||'').startsWith('video');
  document.getElementById('goMediaPreview').innerHTML=(x.preview_url?(video?'<video src="'+E(x.preview_url)+'" controls style="width:100%;max-height:65vh"></video>':'<img src="'+E(x.preview_url)+'" style="width:100%;max-height:65vh;object-fit:contain">'):'<div class="go-empty">Preview unavailable.</div>')+'<p class="go-modal-note">'+E(x.caption||x.metadata?.original_name||'Media')+' · '+fmt(x.created_at)+'</p><div class="go-actions" style="justify-content:flex-end"><button class="btn go-danger" id="goPreviewDelete">Delete Media</button></div>';document.getElementById('goPreviewDelete').onclick=()=>deleteMedia(x.id);
...
async function saveGuide(){
  ...
  const display_name=document.getElementById('goGName').value.trim(),email=document.getElementById('goGEmail').value.trim().toLowerCase()||null,phone=normalizeUSPhone(document.getElementById('goGPhone').value),notes=document.getElementById('goGNotes').value.trim()||null,active=document.getElementById('goGActive').checked;
  if(!display_name||!phone)return notice('Guide name and mobile number are required.');
...
async function bulkImportGuides(){
  if(!live())return notice('Connect the trial backend to import guides.');
  const raw=document.getElementById('goBulkText').value.trim();
  if(!raw)return notice('Paste the guide roster first.');
  const lines=raw.split(/\r?\n/).map(x=>x.trim()).filter(Boolean);
  const parsed=[],errors=[];
  lines.forEach((line,i)=>{
    const parts=(line.includes('\t')?line.split('\t'):line.split(',')).map(x=>x.trim());
    const name=parts[0]||'',phone=normalizeUSPhone(parts[1]||''),email=(parts[2]||'').toLowerCase()||null;
    if(!name||!phone)errors.push('Line '+(i+1)+': needs name and valid mobile number');
    else parsed.push({name,phone,email});
  });
  if(errors.length){document.getElementById('goBulkResult').innerHTML='<span style="color:#9a3e30">'+E(errors.slice(0,8).join(' • '))+'</span>';return}
  const existingPhones=new Set(guides.map(g=>normalizeUSPhone(g.phone)).filter(Boolean));
  let added=0,authorized=0,skipped=0;
  for(const row of parsed){
    let guide=guides.find(g=>normalizeUSPhone(g.phone)===row.phone);
    if(!guide){
      const r=await sb.from('dh_guides').insert({org_id:liveOrgId,display_name:row.name,phone:row.phone,email:row.email,active:true,created_by:liveUser.id}).select().single();
      if(r.error){errors.push(row.name+': '+r.error.message);continue}
      guide=r.data;guides.push(guide);existingPhones.add(row.phone);added++;
    }else skipped++;
    if(document.getElementById('goBulkAuthorize').checked&&weekendId){
      const a=await sb.from('dh_guide_weekend_access').upsert({org_id:liveOrgId,weekend_id:weekendId,guide_id:guide.id,status:'active',created_by:liveUser.id},{onConflict:'weekend_id,guide_id'});
      if(!a.error)authorized++;
      else errors.push(row.name+' authorization: '+a.error.message);
    }
  }
  await loadAll();
  document.getElementById('goBulkResult').innerHTML='<b>'+added+' added</b> · '+skipped+' already existed'+(weekendId?' · '+authorized+' authorized for this weekend':'')+(errors.length?'<br><span style="color:#9a3e30">'+E(errors.slice(0,5).join(' • '))+'</span>':'');
  notice('Guide roster import complete.');
}
...
function subscribe(){
  ...
  realtimeChannel=sb.channel('guideops-v2-'+weekendId)
    .on('postgres_changes',{event:'INSERT',schema:'public',table:'dh_hunt_messages',filter:'weekend_id=eq.'+weekendId},payload=>{handleOpsIncomingMessage(payload.new);scheduleReload()})
    ...
    .on('postgres_changes',{event:'INSERT',schema:'public',table:'dh_media_assets',filter:'org_id=eq.'+liveOrgId},scheduleReload)
    .subscribe();
}
